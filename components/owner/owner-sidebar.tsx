...
import { BarChart3, BookMarked, Key, LogOut, Menu, ParkingCircle, Settings, User } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

interface OwnerSidebarProps extends React.HTMLAttributes<HTMLDivElement> {}
...
export function OwnerSidebar({ className }: OwnerSidebarProps) {
  ...
  const routes = [
    ...
    },
    {
      ...
      icon: ParkingCircle,
      href: "/owner/slots",
      active: pathname === "/owner/slots",
    },
    {
      label: "Bookings",
      icon: BookMarked,
      href: "/owner/bookings",
      active: pathname === "/owner/bookings",
    },
    {
      label: "Analytics",
      icon: BarChart3,
      href: "/owner/analytics",
      active: pathname === "/owner/analytics",
    },
    {
      label: "Settings",
      icon: Settings,
      href: "/owner/settings",
      active: pathname === "/owner/settings",
    },
  ]
        ...
        <div className="flex h-full flex-col">
          <div className="flex h-14 items-center border-b px-4">
            <Link href="/owner/dashboard" className="flex items-center gap-2 font-semibold">
              ...
              <span className="rounded-md bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-800 dark:bg-purple-900/30 dark:text-purple-300">
                Owner
              </span>
            </Link>
          </div>
